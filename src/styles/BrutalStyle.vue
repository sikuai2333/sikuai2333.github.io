...
const usecases = [
  { id: 'blog', icon: '✎', title: '博客', desc: '技术博客、个人日志、文章发布系统' },
  { id: 'shop', icon: '◈', title: '电商', desc: '商品展示、购物车、结账流程' },
  { id: 'docs', icon: '☰', title: '文档', desc: '技术文档、API参考、知识库' },
  { id: 'tools', icon: '⚙', title: '工具站', desc: '在线工具、计算器、转换器' },
  { id: 'portfolio', icon: '✦', title: '作品集', desc: '个人作品展示、项目案例' },
  { id: 'dashboard', icon: '◧', title: '后台', desc: '数据面板、管理系统、分析工具' },
]
</script>
  ...
  <section class="usecases">
    ...
    <div class="usecase-grid">
      <a v-for="u in usecases" :key="u.id" :href="'/2/' + u.id" class="usecase-card" @click.prevent="$router.push('/2/' + u.id)">
        ...
        <h3>{{ u.title }}</h3>
        <p>{{ u.desc }}</p>
        <span class="usecase-cta">查看示例 →</span>
      </a>
    </div>
  </section>
...
<style scoped>
.page{background:#f5f0e8;color:#1a1a1a;min-height:100vh;padding-top:var(--nav-height);transition:background 0.3s,color 0.3s}
...
.hero{text-align:center;padding:0 20px 60px;max-width:1080px;margin:0 auto}
...
.toggles,.progress-sec,.tags-sec{max-width:1080px;margin:0 auto;padding:0 20px 32px}
...
.cards{max-width:1080px;margin:0 auto;padding:0 20px 32px;display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px}
...
.cta-sec{max-width:1080px;margin:0 auto;padding:0 20px 48px;display:flex;gap:12px;flex-wrap:wrap}
...
.usecases{max-width:1080px;margin:0 auto;padding:0 20px 48px}
.usecases h2{font-size:18px;font-weight:800;text-transform:uppercase;margin-bottom:8px;letter-spacing:0.05em}
.subtitle{font-size:14px;color:#666;margin-bottom:20px}
.usecase-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px}
.usecase-card{padding:24px;border:3px solid #1a1a1a;background:#fff;box-shadow:6px 6px 0 #1a1a1a;transition:all 0.15s;text-decoration:none;color:#1a1a1a;display:block}
.usecase-card:hover{transform:translate(3px,3px);box-shadow:3px 3px 0 #1a1a1a}
.usecase-icon{font-size:24px;display:block;margin-bottom:10px}
.usecase-card h3{font-size:15px;font-weight:800;margin-bottom:4px;text-transform:uppercase}
.usecase-card p{font-size:12px;color:#666;margin-bottom:12px}
.usecase-cta{font-size:12px;font-weight:800;color:#ff6b35}

footer{text-align:center;padding:40px 20px 80px;font-size:13px;color:#999;letter-spacing:0.1em}
@media(max-width:768px){.hero .slogan{font-size:clamp(22px,4vw,36px)}.cards{grid-template-columns:repeat(auto-fill,minmax(260px,1fr))}.usecase-grid{grid-template-columns:repeat(auto-fill,minmax(260px,1fr))}.cta-sec{flex-direction:column}.big-btn{width:100%}}@media(max-width:480px){.cards,.usecase-grid{grid-template-columns:1fr}.hero .slogan{font-size:22px}.hero .clock{font-size:28px}}
</style>
